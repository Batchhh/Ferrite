...
pub(super) fn emit_type_header(td: &TypeDef, out: &mut String) {
    let flags = td.flags;
...
pub(super) fn type_visibility(flags: u32) -> &'static str {
    match flags & 0x07 {
...
pub(super) fn type_kind_keyword(kind: &TypeKind) -> &'static str {
    match kind {
...
pub(super) fn is_implicit_base(base: &str, kind: &TypeKind) -> bool {
    match kind {
...
pub(super) fn emit_custom_attributes(attrs: &[CustomAttribute], out: &mut String, indent: &str) {
    for attr in attrs {
        out.push_str(indent);
        out.push('[');
...
pub(super) fn emit_enum_members(td: &TypeDef, out: &mut String) {
