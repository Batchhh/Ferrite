...
pub(super) fn rewrite_property_accessors(stmts: Vec<Statement>) -> Vec<Statement> {
    stmts.into_iter().map(rewrite_prop_stmt).collect()
}

pub(super) fn rewrite_prop_stmt(stmt: Statement) -> Statement {
    match stmt {
...
pub(super) fn rewrite_prop_expr(expr: Expr) -> Expr {
    match expr {
