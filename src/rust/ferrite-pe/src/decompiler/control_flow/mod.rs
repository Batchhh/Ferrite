...
mod tests {
    #[test]
    fn test_decompile_with_branches() {
        ...
        if let Ok(data) = std::fs::read(dll_path) {
            ...
            println!(
                "{}/{} types, if={}, try={}, while={}, switch={}",
                success, total, if_count, try_count, while_count, switch_count
            );
            assert!(if_count > 0, "Should find if statements in real DLLs");
            assert!(try_count > 0, "Should find try/catch in real DLLs");
            assert!(
                switch_count > 0,
                "Should find switch statements in real DLLs"
            );
        }
    }
}
